...
export function WorkspaceGate({children}:{children:ReactNode}){
 ...
 const load=async()=>{if(!supabase){setError('Supabase is not configured.');setLoading(false);return}setLoading(true);setError('');const {data:{session}}=await supabase.auth.getSession();setSignedIn(!!session);if(!session){setLoading(false);return}const {data:p,error:pe}=await supabase.from('profiles').select('*').eq('auth_user_id',session.user.id).maybeSingle();if(pe){setError(pe.message);setLoading(false);return}setProfile(p);if(p){const {data:m,error:me}=await supabase.from('organization_members').select('role,organizations(*)').eq('user_id',p.id).eq('status','active').maybeSingle();if(me)setError(me.message);setMembership(m)}setLoading(false)};
